...
(function () {
  ...
  function setHeadH() {
    /* offer bar + header are separate sibling sections; publish the offer bar's
       height so the header can stick flush right below it (CSS --aa-offer-h) */
    var offer = document.querySelector('.announcement-bar-section') || document.querySelector('.announcement-bar');
    if (offer) document.documentElement.style.setProperty('--aa-offer-h', Math.round(offer.getBoundingClientRect().height) + 'px');
  }
  function initHeader() {
    /* the offer bar is the sticky anchor at top:0; the header sticks flush below it */
    var anchor = document.querySelector('.announcement-bar-section') || document.querySelector('.shopify-section-group-header-group') || document.querySelector('.section-header');
    if (anchor) anchor.classList.add('aa-sticky');
    setHeadH();
    if (headerScrollBound) return;       /* bind listeners ONCE (survives Turbo visits) */
